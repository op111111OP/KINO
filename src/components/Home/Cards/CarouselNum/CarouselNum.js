...
function CarouselNum({ valuesPage, page }) {
  const pageNum = page * 46 - 552;
  ...
  useEffect(() => {
    ...
    const newNumbers = [];
    for (let numN = startNum; numN <= endNum; numN++) {
      newNumbers.push(numN);
    }
    setRenderedNumbers(newNumbers);
  }, [page]);

  useEffect(() => {
    valuesPage(selectedNum);
    window.scrollTo(0, 0);
  }, [selectedNum]);
  ...
  //   console.log(leftPosition, pageNum);
  ...
  const handleNextClick = () => {
    ...
    if (leftPosition > -pageNum) {
      setLeftPosition(-pageNum);
    }
  };
